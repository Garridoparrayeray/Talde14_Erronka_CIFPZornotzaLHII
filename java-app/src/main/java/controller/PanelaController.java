...
public class PanelaController implements Initializable {
    ...
    @FXML
    public void langileaBerriaSarbideAzkarra() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/LangileBerria.fxml"));
            Node nodoa = loader.load();
            
            // Kontroladoreari contentArea pasatzen diogu, gero itzuli ahal izateko
            LangileBerriController ctrl = loader.getController();
            
            // OHadminARRA: Ziurtatu klase honetan `@FXML private StackPane contentArea;` 
            // definituta daukazula, LangileakController-en bezala.
            ctrl.setContentArea(contentArea);
            
            // Panela kargatzen dugu
            UIKudeatzailea.kargatuPanela(contentArea, nodoa);
            
        } catch (Exception e) {
            System.err.println("Errorea langileaBerriaSarbideAzkarra kargatzean: " + e.getMessage());
            e.printStackTrace();
        }
    }
}
